...
export function deleteHistoryRecord(id) {
  ...
  const nextTraining = training.filter((item) => item.id !== id);
  const nextDiagnosis = diagnosis.filter((item) => item.id !== id);
  const changed = nextTraining.length !== training.length || nextDiagnosis.length !== diagnosis.length;
  if (changed) {
    write(TRAINING_KEY, nextTraining);
    write(DIAGNOSIS_KEY, nextDiagnosis);
  }
  return changed;
}

export function clearHistory(type) {
  if (type === 'diagnosis') return write(DIAGNOSIS_KEY, []);
  if (type === 'training') return write(TRAINING_KEY, []);
  return write(TRAINING_KEY, []) && write(DIAGNOSIS_KEY, []);
}
